...
def save_image(elevation_matrix, minimum, maximum,image='/home/kgacek/image3.bmp'):
...
def main():
    key_it=0
    gmaps = googlemaps.Client(key=keys[key_it])
   ...
   # lx, ly = generate_arrays(tuple(gmaps.geocode("Ligota Ksiazeca 11a")[0]['geometry']['location'].values()))
    ...
    maximum=max(maximum)
    minimum=min(minimum)
    save_image(elevation_matrix,minimum,maximum)
